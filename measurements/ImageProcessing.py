...
def fitProjection(projection, guess_vec):
    '''
    ...
    '''
    opt_gauss, cov_gauss = scipy.optimize.curve_fit(gauss, range(len(projection)), projection, maxfev = 1000000, p0=guess_vec, bounds = [[0,0,0,0],[100000, 5000, 300, 100000]] )
...
def VCCAnalysis(image, plotting = True, amp = 5000, sigma = 50, offset = 200, npix = 150, resolution = 1, verbose = False):
    '''
    ...
    ## sets up initial guess for Gaussian fitting
    fit_initial = [amp, npix, sigma, offset]
    
    # x and y projections calculated and fit, with slight length correction
    xproj = makeXProjection(img)
    if np.remainder(len(xproj),2) == 1:
        xproj = xproj[:-1]
    xgauss_fit, xcov, xerr =  fitProjection(xproj, fit_initial)
    ...
    ygauss_fit, ycov, yerr =  fitProjection(yproj, fit_initial)
    ydata = makeGaussFitData(yproj, ygauss_fit)
    masked = makeMaskedImage(img)
...
def YAGAnalysis(image, plotting = True, amp = 5000, sigma = 50, offset = 200, npix = 150, resolution = 1, verbose = False):
    '''
    ...
    newimg = image[int(xcm-xnpix/2):int(xcm+xnpix/2),int(ycm-ynpix/2):int(ycm+ynpix/2)]
    ...
    return img, masked, xproj, xdata, xgauss_fit, xcov, xerr, yproj, ydata, ygauss_fit, ycov, yerr
def doAnalysisAndReturnDict(images, plotting = True, VCC = True, npix = 150, resolution = 1, verbose = False):
    '''
    ...
    for i in range(np.shape(images)[2]):
        ...
        if VCC:
            ...
            img, masked, xproj, xdata, xgauss_fit, xcov, xerr, yproj, ydata, ygauss_fit, ycov, yerr = YAGAnalysis(img, plotting = plotting, resolution = resolution, npix = npix, verbose = verbose)
...
def package(vcc, vccpvs, vcc_res, yag, yagpvs, yag_res, pvnames):
    ...
    return inputs, vcc_images, yag_images, np.array(yag_fits)