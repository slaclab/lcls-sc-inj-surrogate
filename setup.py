        ...
        for variable in data["output_variables"]:
            ...
            if variable_data["variable_type"] == "scalar":
                output_variables[variable] = ScalarOutputVariable(
                    name=variable,
                    default=variable_data.get("default", None),    
                    units=variable_data.get("units"),
                    parent=variable_data.get("parent"),
                )
...
if __name__ == "__main__":
    import numpy as np
    var_file = "binder/files/surrogate_model_variables.pickle"

    from lume_model.utils import save_variables
    input_image = np.load("binder/files/CNN_081120_Stock_image.npy")
    input_variables, output_variables = build_variables_from_description_file("binder/files/my_description.json", input_extras={"input_image": {"default": input_image}})
